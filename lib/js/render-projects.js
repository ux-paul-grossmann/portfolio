  ...
  function buildFinalDesignCarousel(images, id) {
    ...
    const items = images.map((img, i) => `
      <div class="carousel-item${i === 0 ? ` active` : ``}">
        <img src="${img}" class="d-block w-100 px-0 mx-0 rounded-lg" alt="" style="cursor:pointer"
             data-glightbox-img="${img}" data-glightbox-gallery="carousel-${id}">
      </div>`).join('');
    return `
      <div id="carousel-${id}" class="carousel slide carousel-fade shadow mb-4" data-ride="carousel"
           data-carousel-imgs='${JSON.stringify(images)}'>
...
// At the end of buildCollapseContent function, before return html
html += `
...
// Attach click handler for carousel images → open GLightbox with gallery
document.querySelectorAll('[data-carousel-imgs]').forEach(function(carousel) {
    var imgs;
    try { imgs = JSON.parse(carousel.getAttribute('data-carousel-imgs')); } catch(e) { return; }
    if (!imgs || !imgs.length) return;

    carousel.querySelectorAll('[data-glightbox-img]').forEach(function(img, i) {
        img.addEventListener('click', function(e) {
            e.preventDefault();
            if (window.glightbox) {
                window.glightbox.open({
                    elements: imgs.map(function(src) {
...
// Click handler for carousel images → open GLightbox with gallery
document.addEventListener('click', function(e) {
    var target = e.target.closest('[data-glightbox-img]');
    if (!target) return;
    var carousel = target.closest('[data-carousel-imgs]');
    if (!carousel) return;
    e.preventDefault();
    var imgs;
    try { imgs = JSON.parse(carousel.getAttribute('data-carousel-imgs')); } catch(err) { return; }
    if (!imgs || !imgs.length) return;
    var allImgs = carousel.querySelectorAll('[data-glightbox-img]');
    var index = Array.prototype.indexOf.call(allImgs, target);
    if (window.glightbox) {
        window.glightbox.open({
            elements: imgs.map(function(src) { return { href: src, type: 'image' }; }),
