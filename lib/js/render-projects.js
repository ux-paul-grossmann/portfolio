  ...
  function buildFinalDesignCarousel(images, id) {
    ...
    const items = images.map((img, i) => `
      <div class="carousel-item${i === 0 ? ` active` : ``}">
        <a href="${img}" class="glightbox" data-glightbox="type: image; gallery: carousel-${id}">
          <img src="${img}" class="d-block w-100 px-0 mx-0 rounded-lg" alt="">
        </a>
      </div>`).join('');
    return `
...
setTimeout(function() {
    if (window.glightbox) {
        try { window.glightbox.reload(); } catch(e) {}
