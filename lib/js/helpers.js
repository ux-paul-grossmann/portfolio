...
(function(){
    ...
    function apply(){
        ...
        var curCenter = rect.top + rect.height / 2;
        var desired = getGoldenY();
        var delta = desired - curCenter;
        var newMargin = delta;
        if (!isClampOff()) {
            var headerBottom = profile ? profile.getBoundingClientRect().bottom : 0;
            var navTop = nav ? nav.getBoundingClientRect().top : window.innerHeight;
            var newTop = rect.top + delta;
            var newBottom = newTop + rect.height;
            var gap = 12;
            if (newTop < headerBottom + gap) newMargin = headerBottom + gap - rect.top;
            if (newBottom > navTop - gap) {
                if (rect.height < (navTop - headerBottom - gap*2)) {
                    var maxTop = navTop - gap - rect.height;
                    if (rect.top + newMargin > maxTop) newMargin = maxTop - rect.top;
                }
            }
