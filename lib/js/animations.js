        ...
        inView(".kontext-wrap", () => {
            allNotes.forEach((note, i) => {
                ...
                if (pin) {
                    ...
                    { delay: (i * 0.05) + 1.2, duration: 0.4, easing: "ease-out" });
                }
            });
        }, { margin: "-10%" });
    }

    // --- 6. SCROLL TRIGGERS (Bootstrap 4 kompatibel) ---
    const thumbConfigs = [
        { id: "#firstThumbCollapse",  x: -50, delay: 0.05 },
        { id: "#secondThumbCollapse", x: 50,  delay: 0.05 },
        { id: "#thirdThumbCollapse",  x: -50, delay: 0.1  },
        { id: "#fourthThumbCollapse", x: 50,  delay: 0.1  },
        { id: "#fifthThumbCollapse",  x: -50, delay: 0.2  },
        { id: "#sixtThumbCollapse",   x: 50,  delay: 0.2  }
    ];

    inView("#projekte", () => {
        thumbConfigs.forEach(conf => {
            const el = document.querySelector(conf.id);
            if (el) {
                animate(el, 
                    { x: [conf.x, 0], opacity: [0, 1] }, 
                    { delay: conf.delay, duration: 0.5, easing: "ease-out" }
                );
            }
        });
    }, { margin: "-10%" });

    // --- 7. EXPAND ANIMATION (Bootstrap 4 multi-collapse) ---
